...
class LexemesControllerTest < ActionController::TestCase
  ...
  def test_fixture_should_set_up_correctly
    assert lexemes(:liter_lex).headwords.count.positive?, 'Test fixture should have more than zero headwords'
    assert loci(:nemo).attests?('litre'), "Fixture loci(:nemo) should attest 'litre'."
    assert Locus.attesting(lexemes(:liter_lex)).count.count.positive?, "Loci should attest the 'liter' lexeme."

    assert lexemes(:liter_lex).loci.count.count.positive?, 'Test fixture does not have any loci for further tests'
  end
  ...
  def test_show_should_assign_loci_for
    ...
    lexemes(:liter_lex).headword_forms.each do |headword|
      assert loci_for.key?(headword), "@loci_for hash should have a key for #{headword}"
  ...
  def test_show_by_headword_can_return_multiple_results
    ...
    assert_not_nil lexeme,
                   "Headword 'liter' should return a lexeme; 'liter's lexeme is #{Lexeme.lookup_by_headword('liter')}"
  ...
  def test_substring_search_friendly_url
    assert_recognizes(
      { controller: 'lexemes', action: 'matching', headword: 'liter',
        matchtype: Lexeme::SUBSTRING }, '/lexemes/matching/liter'
    )
  ...
  test 'should handle etymology sources in edit form correctly' do
    ...
    assert_no_difference('Source.count') do
      put :update,
          params: { id: lexeme.id, commit: I18n.t('lexemes.form.save_and_continue_editing'),
                    lexeme: lexeme.attributes.except('id', 'created_at', 'updated_at') }
      assert_select '.subentry', subentry_count
      assert_select '.etymothesis .source', etymo_count
