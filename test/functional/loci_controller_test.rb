...
class LociControllerTest < ActionController::TestCase
  ...
  def test_should_create_locus
    assert_difference('Locus.count') do
      post :create, params: { :locus => { :example => "" }, :authorship => { :title_id => "", :author_id => "" }, :new_title => {:name => "Eggs"}, :new_author => {:name => "Bacon"} }
    end
  ...
  def test_should_show_locus
    get :show, params: { :id => loci(:one).id }
    assert_response :success
  end

  def test_should_get_edit
    get :edit, params: { :id => loci(:one).id }
    assert_response :success
  end

  def test_should_update_locus
    put :update, params: { :id => loci(:one).id, :locus => { :example => "" }, :authorship => { :title_id => "", :author_id => "" }, :new_title => {:name => "Eggs"}, :new_author => {:name => "Bacon"} }
    assert_redirected_to locus_path(assigns(:locus))
  end

  def test_should_destroy_locus
    assert_difference('Locus.count', -1) do
      delete :destroy, params: { :id => loci(:one).id }
    end
  ...
  test "@headwords should be initial case insensitive" do
    get :edit, params: { :id => loci(:wrong_case_locus) }
  ...
  test "should get unattached" do
  	get :unattached, params: { :id => loci(:nemo).id }
  	
  	assert_response :success
  end
  
  test "should get author search" do
    get :matching, params: { :author => authors(:one).id }
  ...
  test "should be able to create new interpretations by nested attributes" do
      ...
      put :update, params: { id: loci(:one).id, locus: 
