...
class EtymologiesHelperTest < ActionView::TestCase
  ...
  test 'chained etymologies, simple case' do
    ...
    assert_equal "#{ETYMON % '1ary'} #{GLOSS % 'primary'}, from #{ETYMON % '2ary'}.",
                 html_format(etymologies(:chained_A))
  end

  test 'chained etymologies, compound case' do
    assert_equal html_escape('1ary A "test" + 1ary B "test 2"; where 1ary A is from 2ary A, and where 1ary B is from 2ary B.'),
                 wiki_format(etymologies(:chained_B))
    assert_equal "#{ETYMON % '1ary A'} #{GLOSS % 'test'} + #{ETYMON % '1ary B'} #{GLOSS % 'test 2'}; where #{ETYMON % '1ary A'} is from #{ETYMON % '2ary A'}, and where #{ETYMON % '1ary B'} is from #{ETYMON % '2ary B'}.",
                 html_format(etymologies(:chained_B))
  end
  ...
  test 'issue #117 - anomalous output' do
    words = [%i[abdomen abdo], [:abdo, %i[ab do]], %i[ab apo], %i[do dh3], [:apo], [:dh3]]
    subentries = {}
