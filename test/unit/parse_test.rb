...
class ParseTest < ActiveSupport::TestCase
  ...
  test 'most wanted should be initial-case-insensitive' do
    assert Headword.with_translations.where(form: 'blue').present?
    assert Headword.with_translations.where(form: 'Blue').blank?
  ...
  test 'most_wanted_in(locus, count) should be a functioning scope' do
    ...
    Locus.find_each do |roka|
      ...
      roka_parse = roka.parses.without_entries.inject([]) do |memo, paasu|
        memo.count > paasu.count ? memo : paasu
      end

      if roka_parse.present? && mwi_parse.present?
        assert_equal roka_parse.count, mwi_parse.count,
                     "Locus #{roka.id} does not match - Locus gives #{roka_parse}, MWI gives #{mwi_parse}"
