...
class LexemeTest < ActiveSupport::TestCase
  ...
  test 'lookup_all_by_headwords' do
    results = Lexeme.lookup_all_by_headwords(%w[liter spring])
  ...
  test 'constructions' do
    ...
    assert constructions.include?(lexemes(:with_construction)),
           "#{constructions} does not include #{lexemes(:with_construction)}"
  end
