module LociHelper
  ...
  def sense_select(senses)
    ...
    groups = senses.collect do |sense|
      ...
      new_senses[optgroup] =
        [
          sense.subentry.senses.build(definition: t('helpers.loci.new_sense',
                                                    lang: sense.lexeme.dictionaries.first.try(:language).try(:name))), sense.subentry.id
        ]
  ...
  def cite_locus(locus, counter)
    counter_link = link_to(greek_numeral(counter).to_s, locus, name: "locus-#{locus.id}")
    if @lexeme
      construction_links = render(partial: 'shared/note', collection: @constructions.collect do |constr|
                                                                        constr if constr.loci.include?(locus)
                                                                      end.compact)
    end
    ...
    options = { counter: counter_link, constructions: construction_links, author:, title:,
                pointer: }
    if pointer.present?
      t('helpers.loci.cite_html',
        **options.merge(default: '%{counter} %{constructions} %{author}, <i>%{title}</i> %{pointer}:'))
    else
      t('helpers.loci.cite_without_pointer.html',
        **options.merge(default: '%{counter} %{constructions} %{author}, <i>%{title}</i>:'))
    end
  end
end
