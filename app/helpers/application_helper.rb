module ApplicationHelper
  ...
  def headword_link(parse)
    ...
    if head
      link_to(format(HW_LINK, (' wanted' if is_wanted), html_escape(parse.parsed_form)).html_safe, head)
    else
  ...
  def nested_attributes_for(form_builder, *args)
    ...
    content_for :javascript do
      ...
      args.each do |association|
        ...
        unless @nests[assoc_key]
          content << "\nvar #{assoc_key}_template='#{generate_template(form_builder,
                                                                       association[0].to_sym)}';"
        end
  ...
  def remove_link_unless_new_record(form_builder)
    if form_builder.object.try(:new_record?)
      "#{form_builder.hidden_field(:_destroy)} #{link_to(t('helpers.remove_link'),
                                                         "##{form_builder.object.class.name.underscore}", class: 'remove delete_label')}"
    else
      "<span class=\"type-check\">#{form_builder.check_box(:_destroy)} #{form_builder.label(:_destroy,
                                                                                            t('helpers.delete_checkbox'), class: 'delete_label')}</span>"
    end
  end
  ...
  def spaced_render(options = {})
    ...
    sanitize(coll.to_sentence({
                                words_connector: options[:spacer] || t('helpers.spaced_render.words_connector',
                                                                       default: ', '),
                                last_word_connector: options[:last_spacer] || options[:spacer] || t(
                                  'helpers.spaced_render.last_word_connector', default: ', '
                                ),
                                two_words_connector: options[:dual_spacer] || options[:spacer] || t(
                                  'helpers.spaced_render.two_words_connector', default: ', '
                                )
                              }))
  end
  ...
  def navlink_unless_current(name, options = {}, html_options = {}, &block)
    output = if current_page?(options)
               format('<li class="active"><strong>%s</strong></li>', if block_given?
  ...
  def autocomplete(child, form, options = {})
    ...
    form.fields_for child, child_obj do |child_form|
      ...
      label_tag("#{ref}_search", t("activerecord.models.#{child}")) <<
        ...
        content_tag(:div, nil, id: "#{ref}_choices", class: 'autocomplete',
                               data: { ref:, plural: klass.pluralize }) <<
  ...
  def source(lang = (@langs[:source].first if @langs), &block)
    Globalize.with_locale(lang.try(:iso_639_code), &block)
  end
