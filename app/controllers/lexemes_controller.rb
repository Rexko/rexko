...
class LexemesController < ApplicationController
  ...
  def show
    ...
    @loci = @lexeme.loci(include: { source: { authorship: %i[author title] } })
    ...
    @loci_by = Hash[@authors_of.collect do |construction, authors|
                      [construction, Hash[authors.collect do |author|
                                            [author, Locus.where(id: construction.loci).attesting([construction, @lexeme]).authored_by(author)]
                                          end ]]
                    end ]
  ...
  def show_by_headword
    ...
    if params[:matchtype] == Lexeme::CREATE || @lexeme.blank?
      ...
      respond_to do |format|
        format.html do
          redirect_to action: 'matching', headword: params[:headword], matchtype: params[:matchtype]
        end
      end
    end
  end
  ...
  def new
    ...
    @page_title = if flash[:headword].present?
  ...
  def edit
    ...
    @page_title = t('lexemes.edit.page_title', headwords: Globalize.with_locale(@langs[:source].first.try(:iso_639_code)) do
                                                            view_context.titleize_headwords_for(@lexeme)
                                                          end)
  end
      ...
      if @lexeme.save
        ...
        format.html do
          ...
          when t('lexemes.form.save_and_continue_editing')
            ...
            flash[:notice] =
              t('lexemes.create.create_another_prompt', default: '%{success} %{link}', success: flash[:notice],
                                                        link: view_context.link_to(t('lexemes.create.create_another'), controller: 'lexemes', action: 'new'))
            redirect_to(@lexeme)
          end
        end
      ...
      if @lexeme.update(params.fetch(:lexeme, {}).permit(allowed_params))
        ...
        format.html do
          ...
          when t('lexemes.form.save_and_continue_editing')
            ...
            flash[:notice] =
              t('lexemes.create.create_another_prompt', default: '%{success} %{link}', success: flash[:notice],
                                                        link: view_context.link_to(t('lexemes.update.create_new'), controller: 'lexemes', action: 'new'))
            redirect_to(@lexeme)
          end
        end
        format.xml { head :ok }
  ...
  def destroy
    ...
    redirect_target = if request.env['HTTP_REFERER'].blank? ||
