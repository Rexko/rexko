...
class Headword < ApplicationRecord
  ...
  accepts_nested_attributes_for :phonetic_forms, allow_destroy: true, reject_if: proc { |attributes|
  ...
  scope :unattested, lambda {
                       joins(['LEFT OUTER JOIN "parses" ON "parses"."parsed_form" = "form"']).where({ parses: { parsed_form: nil } })
                     }
  ...
  def self.safe_params
    [:_destroy, :form, :descriptively_ok, :prescriptively_ok, Headword.globalize_attribute_names, {
      phonetic_forms_attributes: {}, notes_attributes: Note.safe_params
    }]
  end
  ...
  def self.lookup_all_by_parse(parse)
    find_all_by_form(parse.parsed_form)
  end

  def self.lookup_by_parse(parse)
    find_by_form(parse.parsed_form)
  end
  ...
  def form(loc = nil)
    if loc
      ...
      self[:form] || form_translations.detect(-> { [] }) { |_k, v| v.present? }[1]
    end
  end
