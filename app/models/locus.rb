class Locus < ActiveRecord::Base
  ...
  def attests? form
    ...
    raw_construes = Locus.possibly_construing_with(parses.collect{|f| f[1]}).includes({:source => {:authorship => [:author, :title]}, :parses => [:translations, {:interpretations => {:sense => :translations}}]}).where(Locus.arel_table[:id].not_eq(self.id))
