...
class Locus < ApplicationRecord
  ...
  scope :sorted, lambda {
                   order(Author.arel_table[:name].asc).order(Title.arel_table[:name].asc).order(Source.arel_table[:pointer].asc).references(:author, :title, :source)
                 }

  # Given an Author or array of Authors, return all loci authored by them.
  scope :authored_by, lambda { |author_array|
                        joins(source: :authorship).where(authorships: { author_id: author_array }).distinct
                      }
  ...
  scope :unattached, lambda { |lexeme|
    joins("INNER JOIN attestations ON attestations.locus_id = loci.id INNER JOIN parses ON (parses.parsable_id = attestations.id AND parses.parsable_type = 'Attestation') LEFT OUTER JOIN interpretations ON interpretations.parse_id = parses.id") \
      .where('interpretations.parse_id' => nil, 'parses.parsed_form' => [*lexeme].collect(&:headword_forms).flatten)
  }
  ...
  accepts_nested_attributes_for :attestations, :source, allow_destroy: true, reject_if: proc { |attributes|
  ...
  def potential_constructions
    parses = attestations.each_with_object([]) do |att, memo|
      att.parses.collect(&:parsed_form).each do |pf|
        memo << [att.attested_form, pf]
      end
    end

    raw_construes = Locus.possibly_construing_with(parses.collect do |f|
                                                     f[1]
    ...
    loop do
      ...
      construes.each do |c_attest, c_parse, c_loci|
        construe = { [parse, c_parse] => { phrase: [format('%s %s', attest, c_attest)], loci: loci & c_loci } }
        potentials.update(construe) do |_key, oldval, newval|
          oldval.merge(newval) do |_key, oldval, newval|
            oldval | newval
          end
        end
      end
    ...
    potentials.delete_if { |_k, v| v[:loci].blank? }
  end
  ...
  def self.loci_by_authors_hash(construction, *authors)
    Hash[authors.collect do |author|
      [author,
