class Lexeme < ApplicationRecord
  ...
  scope :sorted, lambda {
                   includes(headwords: :translations).order(Headword::Translation.arel_table[:form].asc).references(headwords: :translations)
                 }
  ...
  accepts_nested_attributes_for :dictionary_scopes, :dictionaries, :subentries, :headwords, :phonetic_forms, allow_destroy: true, reject_if: proc { |attributes|
  ...
  def self.safe_params
    [:dictionaries, {
      subentries_attributes: Subentry.safe_params, dictionary_ids: [], headwords_attributes: Headword.safe_params
    }]
  end
  ...
  def constructions(from_dictionary = nil)
    ...
    Lexeme.attested_by(loci.collect(&:attestations).flatten,
                       'Attestation').where(conditions).includes(%i[headwords dictionaries subentries]).distinct
  end
  ...
  def self.lookup_all_by_headwords(forms, options = {})
    unless options[:initial_case_insensitive]
    ...
    when EXACT
      Lexeme.joins(headwords: [:translations]).where(['headword_translations.form IN (?)',
                                                      forms]).includes(options[:include])
    end
  end
  ...
  def self.authors_hash(constrs)
    Hash[constrs.collect do |construction|
