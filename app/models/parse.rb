class Parse < ApplicationRecord
  ...
  scope :uninterpreted, lambda {
                          includes(:interpretations).where(Interpretation::NOT_INTERPRETING).references(:interpretations)
                        }

  # Returns the most commonly appearing parses without entries.
  # count = number of results to return (defaults to 1)
  scope :most_wanted, lambda { |count = 1|
  ...
  scope :most_wanted_in, lambda { |loci, count = 1|
    most_wanted(count)
  ...
  accepts_nested_attributes_for :interpretations, allow_destroy: true, reject_if: proc { |attributes|
                                                                                    attributes[:sense_id].blank?
                                                                                  }
  ...
  def self.less_popular_than(times, count = nil)
    Parse.find(:all, select: '"parses"."parsed_form", COUNT("parsed_form") AS count_all',
                     group: '"parses"."parsed_form"', order: 'count_all DESC', limit: count, having: ['"count_all" < ?', times])
  end

  def self.popularity_between(low_bound, high_bound)
    Parse.select('"parses"."parsed_form", COUNT("parsed_form") AS count_all').group('"parses"."parsed_form"').order('count_all DESC').having([
                                                                                                                                               '"count_all" <= ? AND count_all >= ?', high_bound, low_bound
                                                                                                                                             ])
  end
  ...
  def any_form_present?
    if parsed_form.blank? && globalize_attribute_names.select do |k|
                               k.to_s.start_with?('parsed_form')
                             end.all? { |k| self[k].blank? }
