...
Given(/^a test dictionary$/) do
  Dictionary.create(title: 'Test', language: Language.first, source_language: Language.first,
                    target_language: Language.first)
end
...
Then(/^I should be on a search results page with (\d+) results for "(.*?)"$/) do |count, query|
  page.assert_text(:visible, I18n.t('lexemes.matching.results', count: count.to_i, query:))
end
